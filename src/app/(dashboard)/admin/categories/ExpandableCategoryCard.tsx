...
export function ExpandableCategoryCard({
  ...
  useEffect(() => {
    if (expanded && nominees.length === 0 && !loadingNominees) {
      setLoadingNominees(true)
      try {
        const res = await fetch(`/api/admin/categories/${category.id}/nominees?year=${ceremonyYear}`, { cache: 'no-store' })
        const data = await res.json()
        if (data.ok) setNominees(data.items ?? [])
        else showErrorToast(data.error || 'Falha ao carregar indicados')
      } catch (err: any) {
  ...
  return (
    ...
    <div className={expanded ? 'col-span-full w-full' : ''}>
      // Quando expandido, ocupa largura total da página (col-span-full)
      <div className={expanded ? 'col-span-full w-full' : ''}>
        <Card id={`category-${category.id}`} className={expanded ? 'overflow-visible' : 'overflow-hidden'}>
